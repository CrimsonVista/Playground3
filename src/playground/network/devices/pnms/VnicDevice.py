...
from playground.network.protocols.spmp import SPMPClientProtocol
...
class VnicDevice(InterfaceDevice):
    ...
    def query(self, verb, args):
        addr, port = self.tcpLocation()
        loop = asyncio.get_event_loop()
        coro = loop.create_connection(lambda: SPMPClientProtocol(), host=addr, port=port)
        if loop.is_running():
            (transport, protocol) = asyncio.wait_for(coro, timeout=10)
            result, error = asyncio.wait(protocol.query(verb, *args))
        else:
            (transport, protocol) = loop.run_until_complete(coro)
            result, error = loop.run_until_complete(protocol.query(verb, *args))
        if error != None:
